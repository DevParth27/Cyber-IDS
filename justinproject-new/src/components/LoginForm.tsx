...
import { Shield, Lock, Mail, Key, LogIn, AlertTriangle } from 'lucide-react';
...
const LoginForm: React.FC<LoginFormProps> = ({ onSubmit, error, setAuthView }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasSqlInjection) {
      setSecurityAlert('⚠️ SQL Injection attempt detected! Your incident has been logged.');
      return;
    }
    ...
    <div className="w-full max-w-md">
      <div className="bg-white p-8 rounded-lg shadow-md">
        <div className="flex items-center justify-center mb-6">
          ...
          <h2 className="text-2xl font-bold ml-2 text-gray-800">Secure Login</h2>
        </div>
          ...
          <div>
            ...
            >
              <span className="absolute left-0 inset-y-0 flex items-center pl-3">
                <LogIn className="h-5 w-5 text-indigo-500 group-hover:text-indigo-400 transition ease-in-out duration-150" />
              </span>
              Sign in
            </button>
          </div>
        ...
        <div className="mt-6">
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-white text-gray-500 text-center">
                Created by:
                <br />JUSTIN JOSEPH AUGUSTINE
                <br />KAIWALYA S. PUND
                <br />SHALINYA MANWATKAR
                <br />KASHISH KANOJIYA
              </span>
            </div>
          </div>
